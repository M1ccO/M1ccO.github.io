...
from config import EXPORT_DEFAULT_PATH
...
class JawImportMappingDialog(ImportMappingDialog):
    ...
    def _build_ui(self):
        ...
        root.addWidget(self._build_connector_tab(self.GENERAL_FIELDS), 1)
...
class JawExportPage(ExportPage):
    ...
    def export_excel(self):
        default_path = EXPORT_DEFAULT_PATH.with_name('jaw_export.xlsx')
        path, _ = QFileDialog.getSaveFileName(self, 'Export to Excel', str(default_path), 'Excel (*.xlsx)')
        if not path:
            return
        try:
