# NTX Tool Library — Developer and AI Reference
...
NTX Tool Library is a PySide6 desktop application for managing CNC tooling data. It has two independent modules — **TOOLS** and **JAWS** — each with its own database table, service layer, UI page, editor dialog, and export page. Both modules share the same 3D STL preview system.
...
- Applies global QSS stylesheet from `styles/ntx_tool_library_style.qss`
...
All visual styling is driven by Qt property-based CSS in `styles/ntx_tool_library_style.qss`.
...
- the repository includes `ntx_tool_library_settings.json`
