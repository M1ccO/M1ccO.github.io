...
NTX Tool Library gives you two separate, fully integrated modules in a single desktop window:
...
3. Builds the app using `ntx_tool_library.spec`

Output:
```
dist\NTX Tool Library\NTX Tool Library.exe
```
...
.build-venv\Scripts\python.exe -m PyInstaller --noconfirm --clean ntx_tool_library.spec
```
...
Output is written to the `installer-dist` folder. The installer performs a per-user install (no admin rights required) under `%LOCALAPPDATA%\Programs\NTX Tool Library`.

### First-run data location (packaged EXE)

Writable files are stored at:
```
%LOCALAPPDATA%\NTX Tool Library\
```
...
3. build the app with `ntx_tool_library.spec`
...
```text
ntx_tool_library_installer.iss
```
...
```text
%LOCALAPPDATA%\Programs\NTX Tool Library
```
...
1. the built `dist\NTX Tool Library\NTX Tool Library.exe`
2. optionally other shipped `.exe` / `.dll` files
3. the final installer `.exe`

If you want Inno Setup to sign the installer and uninstaller during compile, configure a `SignTool` command and enable the commented `SignTool=` line in `ntx_tool_library_installer.iss`.
