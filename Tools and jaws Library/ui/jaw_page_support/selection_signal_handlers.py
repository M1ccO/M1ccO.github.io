"""Selection and signal handlers for JawPage."""
...
from ui.jaw_catalog_delegate import ROLE_JAW_ID
...
def on_item_selected_internal(page, jaw_id: str, _uid: int) -> None:
    """Handle CatalogPageBase item_selected signal for JawPage."""
    page.current_jaw_id = str(jaw_id or '').strip() or None
...
def on_item_deleted_internal(page, jaw_id: str) -> None:
    """Handle CatalogPageBase item_deleted signal for JawPage."""
    if page.current_jaw_id == jaw_id:
        page.current_jaw_id = None
        page._current_item_id = None
        page.populate_details(None)
    page._sync_detached_preview(show_errors=False)
    page.jaw_deleted.emit(jaw_id)
