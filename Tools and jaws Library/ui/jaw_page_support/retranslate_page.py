...
from ui.jaw_page_support.bottom_bars_builder import retranslate_bottom_bars
from ui.jaw_page_support.topbar_builder import retranslate_filter_toolbar
...
def apply_jaw_page_localization(
    ...
    """Refresh all user-visible strings on JawPage after a locale change."""
    ...
    if hasattr(page, 'selector_hint_label'):
        page.selector_hint_label.setText(
            page._t('tool_library.selector.jaw_hint', 'Drag jaws from the catalog to SP1 or SP2.')
        )
    if hasattr(page, 'selector_header_title_label'):
        page.selector_header_title_label.setText(page._t('jaw_library.selector.header_title', 'Jaw Selector'))
    if hasattr(page, 'selector_module_value_label'):
        page.selector_module_value_label.setText(page._t('tool_library.selector.jaws', 'Jaws'))
    if hasattr(page, 'selector_sp1_slot'):
        page.selector_sp1_slot.set_title(page._t('jaw_library.selector.sp1_slot', 'SP1 jaw'))
    ...
    if hasattr(page, 'selector_sp2_slot'):
        page.selector_sp2_slot.set_title(page._t('jaw_library.selector.sp2_slot', 'SP2 jaw'))
