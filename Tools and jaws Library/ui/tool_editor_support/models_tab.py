"""Build the 3D Models tab for the tool editor dialog.

This module keeps the tab construction in one place so the main dialog can stay
focused on behavior while still owning the widgets and their callbacks.
"""
...
def _build_transform_controls(dialog: Any, model_table: PartsTable) -> QFrame:
    ...
    style_icon_action_button(
        ...
        dialog._t('tool_editor.transform.move', 'SIIRRÄ'),
    )
    style_icon_action_button(
    ...
    style_icon_action_button(
        ...
        dialog._t('tool_editor.transform.reset', 'NOLLAA'),
    )
...
def build_models_tab(dialog: Any, root_tabs: QTabWidget) -> QWidget:
    """Construct the 3D models tab and attach its widgets to *dialog*."""
    models_tab = QWidget()
    ...
    dialog._transform_frame = _build_transform_controls(dialog, dialog.model_table)

    model_btn_bar = create_titled_section(
        dialog._t('tool_editor.models.actions_title', 'Mallien toiminnot')
    )
